...
public class XoaNhanVien extends JFrame implements ActionListener
{
	...
	JLabel name,date,Sdt,address,ma_nv,chuc_vu,ma_chuc_vu,account,password,gioi_tinh,l11,ln;
	JButton b_open,b_delete,b_clear;
...
//Cột 2
		...
		tai_khoan.setBounds(670,220,150,25);
		...
		mat_khau.setBounds(670,260,100,25);
		...
		ChucVu.addActionListener(new ActionListener()
	   	{
		   	public void actionPerformed(ActionEvent ae)
		   	{
			   	String Chuc_Vu =(String)ChucVu.getSelectedItem();
		...
		b_delete = new JButton("Xóa",new ImageIcon("images//delete.png"));
        	b_delete.setBounds(350,330,110,35);b_delete.setToolTipText("Click để xóa nhân viên");
 	...
 	public void actionPerformed(ActionEvent ae)
 	{
  		if(ae.getSource()==b_open)
  		{
  			if((ma_NV.getText()).equals(""))
	        	{
	  	 		JOptionPane.showMessageDialog(this,"Để xóa nhân viên, hãy nhập mã nhân viên!","Warning!!!",JOptionPane.WARNING_MESSAGE);
	        	}
	        	else
	        	{
	        		...
	        		{//fetch
	        			...
	        			while(rs.next())
	        			{
	        				ten.setText(rs.getString(1));
	        				ngay_sinh.setText(rs.getString(2));
	        				sdt.setText(rs.getString(3));
	        				dia_chi.setText(rs.getString(4));
	        				ma_NV.setText(rs.getString(5));
	        				ChucVu.setSelectedItem(rs.getString(6));
	        				ma_CV.setText(rs.getString(7));
	        				tai_khoan.setText(rs.getString(8));
	        				mat_khau.setText(rs.getString(9));
  		...
  		else if(ae.getSource()==b_delete)
  		{
  			if(((ma_NV.getText()).equals("")))
  		     	{
  				JOptionPane.showMessageDialog(this,"Hãy điền mã nhân viên !","Warning!!!",JOptionPane.WARNING_MESSAGE);
  		     	}
	        	...
	        	{ 
					...
					//Class.forName("com.mysql.jdbc.Driver");
	        			...
	        			ps=con.prepareStatement("delete from mobileshop.nhanvien where name='"+ten.getText()+"' or MaNV='"+ma_NV.getText()+"'");
	        			ps.executeUpdate();
	        			JOptionPane.showMessageDialog(null, "Nhân viên đã bị xóa");
	   			...
	   			{
		   			System.out.println(se);
		   J			OptionPane.showMessageDialog(null,"SQL Error:"+se);
				 }
			   	catch(Exception e)
	   			{
		   			System.out.println(e);
